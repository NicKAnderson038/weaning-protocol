(function () {
    ...
    Lungs.$inject = ['$scope', '$state', '$sessionStorage','factoryService'];

    function Lungs($scope, $state, $sessionStorage, factoryService) {
        var vm = this;
    ...
    function _toggle(x) {
        ...
        if (x.toggle) {
            ...
            toastr.error("FiO2 is incorrect. \nRe-enter FiO2.");
            return;
        } else if (x.id === 1 && x.count.charAt(0) === "2" && x.count.charAt(1) === "0") {
            toastr.error("FiO2 20%? \nOnly possible in a hypoberic Chamber.");
            return;
            ...
            return;
        } else if (x.id === 3 && (x.count.length > 2 || x.count.length < 2 || x.count.charAt(0) === "0" || x.count.charAt(0) === "7" || x.count.charAt(0) === "8" || x.count.charAt(0) === "9")) {
            toastr.error("PIP is incorrect. \nRe-enter PIP.");
            return;
        } else if (x.id === 4 && (x.count.length > 2 || x.count.length < 1 || x.count.charAt(1) === "3" || x.count.charAt(1) === "4" || x.count.charAt(1) === "5" || x.count.charAt(1) === "6" || x.count.charAt(1) === "7" || x.count.charAt(1) === "8" || x.count.charAt(1) === "9")) {
            toastr.error("PEEP is incorrect. \nRe-enter PEEP.");
            return;
        } else if (x.id === 5 && (x.count.length > 2 || x.count.length < 1 || x.count.charAt(0) === "0")) {
            toastr.error("Platue Pressure is incorrect. \nRe-enter Platue.");
            return;
        } else {
            console.log("close");
            console.log(x.count);
