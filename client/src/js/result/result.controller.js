(function() {
  ...
  function Result($scope, $state, $sessionStorage, factoryService) {
    ...
    function activate() {
      console.log("formdata result", vm.$sessionStorage.formData);
      ...
      var t2 = new Date();
      var datestring2 =
        ("0" + (t2.getMonth() + 1)).slice(-2) +
        "-" +
        ("0" + t2.getDate()).slice(-2) +
        "-" +
        t2.getFullYear() +
        " " +
        ("0" + t2.getHours()).slice(-2) +
        ":" +
        ("0" + t2.getMinutes()).slice(-2);
      ...
      for (let key in vm.lineStacked.data) {
        console.log(
          `DataChunk in action ${vm.lineStacked.data[key]["column1"]}`
        );
      }
    }
    ...
    function _test() {
      ...
      for (let key in vm.lineStacked.data) {
        ...
        console.log(
          `DataChunk in action ${vm.lineStacked.data[key]["column3"]}`
        );
      }
    }
  }
