(function() {
  ...
  function Wean($scope, $state, $sessionStorage, FormDataModel, WeanService) {
    ...
    if (
      vm.$sessionStorage.totalData == undefined ||
      vm.$sessionStorage.totalData == null
    ) {
      ...
      console.log(
        `Continued session!! Continued session!! Continued session!! ${
          vm.$sessionStorage.totalData
        }`
      );
    ...
    const data = {
      clinicianName: "Tester Three",
      clinicianTitle: "Title Three",
      patientList: []
    };

    WeanService.post(data)
      .then(response => {
        console.log(response);
      })
      .catch(error => {
